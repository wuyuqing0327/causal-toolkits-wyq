...
class UpliftTreeClassifier:
    ...
    def __init__(self, max_features=None, max_depth=3, min_samples_leaf=100,
                 ...
                 control_name='control', normalization=True, is_constrained=True,
                 is_continuous_method=False, show_log=False):
        ...
        elif evaluationFunction == 'DW':
            self.evaluationFunction = self.evaluate_DW
        elif evaluationFunction == 'JS':
            self.evaluationFunction = self.evaluate_JS
        elif evaluationFunction == 'WAS':
            self.evaluationFunction = self.evaluate_WAS
        ...
        elif evaluationFunction == "VAR":
            if is_continuous_method:
                self.evaluationFunction = self.evaluate_VAR_continuous
            else:
                self.evaluationFunction = self.evaluate_VAR
        ...
        elif evaluationFunction == "BPR":
            if is_continuous_method:
                self.evaluationFunction = self.evaluate_BPR_continuous
            else:
                self.evaluationFunction = self.evaluate_BPR
        elif evaluationFunction == "HL":
            self.evaluationFunction = self.evaluate_HL
        else:
            raise ValueError("分裂指标错误")
    ...
    def fit(self, X, treatment, y):
        ...
        # 返回训练好的树模型
        fitted_uplift_tree = self.growDecisionTreeFrom(
    ...
    def js_divergence(pk, qk):
        ...
        M = (pk + qk) / 2.0
        M1 = (1 - pk + 1 - qk) / 2.0
        S = 0.5 * entropy([pk], [M]) + 0.5 * entropy([qk], [M]) + 0.5 * entropy([1 - pk], [M1]) + 0.5 * entropy([1 - qk], [M1])
        return S

    @staticmethod
    def was_divergence(pk, qk):
        if qk < 0.1 ** 6:
            qk = 0.1 ** 6
        elif qk > 1 - 0.1 ** 6:
            qk = 1 - 0.1 ** 6
        from scipy.stats import wasserstein_distance
        S = wasserstein_distance([pk], [qk]) + wasserstein_distance([1 - pk], [1 - qk])
        return S
    ...
    def evaluate_DW(self, nodeSummary, control_name):
        """
        同时考虑组间和组内距离
        :param nodeSummary:
        :param control_name:
        ...
        res = []
        d_res = 0
        for treatment_group in sorted(nodeSummary.keys(), reverse=True):
            if treatment_group != control_name:
                res.append(nodeSummary[treatment_group][0] - pc)
                d_res += nodeSummary[treatment_group][0] * np.log(nodeSummary[treatment_group][0] / pc)
        if len(res) > 1:
            d_r = 0
            for j in range(1, len(res)):
                d_r += res[j] * np.log(res[j] / res[j - 1])
            return d_res * d_r
        else:
            return d_res

    def evaluate_BPR(self, nodeSummary, control_name):
        """
        借鉴推荐BPR Loss
        :param nodeSummary:
        :param control_name:
        ...
        pc = nodeSummary[control_name][0]
        d_res = 0
        for treatment_group in nodeSummary.keys():
            if treatment_group != control_name:
                d_res += - np.log(self.sigmoid(nodeSummary[treatment_group][0] - pc))
        return d_res

    def evaluate_BPR_continuous(self, nodeSummary, control_name):
        """
        借鉴推荐BPR Loss
        :param nodeSummary:
        :param control_name:
        ...
        for treatment_group in nodeSummary.keys():
            if treatment_group != control_name:
                res.append(nodeSummary[treatment_group][0])
                d_res += - np.log(self.sigmoid(nodeSummary[treatment_group][0] - pc)) - np.log(self.sigmoid(pc - nodeSummary[treatment_group][0]))
        if len(res) > 1:
            d_r = 0
            for j in range(1, len(res)):
                d_r += - np.log(self.sigmoid(res[j] - res[j - 1]))
            return d_res + d_r
        else:
            return d_res
    ...
    def group_inner_distance(self, res):
        """
        计算组内距离
        :param res:
        :return:
        """
        assert len(res) > 1, '数组应该长度大于1'
        dis = 0
        for i in res:
            for j in res:
                if i != j:
                    dis += self.kl_divergence_s(i, j)
        return dis

    def evaluate_HL(self, nodeSummary, control_name):
        '''
        待优化。。。
        '''
        if control_name not in nodeSummary:
        ...
        for r in nodeSummary.keys():
            if r != control_name:
                mu = max(mu, nodeSummary[r][0])
        return mu - pc

    def evaluate_JS(self, nodeSummary, control_name):
        """
        优化KL散度不对称问题
        :param nodeSummary:
        :param control_name:
        ...
        for treatment_group in nodeSummary:
            if treatment_group != control_name:
                d_res += self.js_divergence(nodeSummary[treatment_group][0], pc)
        return d_res

    def evaluate_VAR(self, nodeSummary, control_name):
        """
        考虑Treatment和Control组之间的差异越大越好，同时保证Treatment之间的距离越大越好
        :param nodeSummary:
        :param control_name:
        :return:
        """
        ...
        for treatment_group in sorted(nodeSummary.keys(), reverse=True):
            if treatment_group != control_name:
                res.append(nodeSummary[treatment_group][0] - pc)
        if len(res) > 1:
            return np.var(res) * np.mean(res)
        else:
            return res[0]

    def evaluate_VAR_continuous(self, nodeSummary, control_name):
        """
        考虑Treatment和Control组之间的差异越大越好，同时保证Treatment之间的距离越大越好
        :param nodeSummary:
        :param control_name:
        ...
        for treatment_group in sorted(nodeSummary.keys(), reverse=True):
            if treatment_group != control_name:
                ...
                d_res += self.kl_divergence(nodeSummary[treatment_group][0], pc)
        if len(res) > 1:
            return self.group_inner_distance(res) + d_res
        else:
            return d_res

    def evaluate_WAS(self, nodeSummary, control_name):
        """
        引入推土机距离
        :param nodeSummary:
        :param control_name:
        ...
        for treatment_group in nodeSummary:
            if treatment_group != control_name:
                d_res += self.was_divergence(nodeSummary[treatment_group][0], pc)
        return d_res
    ...
    @staticmethod
    def evaluate_Cape(y, treatment):
        t_groups = list(set(treatment) - {"control"})
        t_groups.sort(reverse=False)
        treat_dict = dict()
        treat_dict['control'] = 0
        for i in range(len(t_groups)):
            treat_dict[t_groups[i]] = i + 1
        w = treatment.apply(lambda x: treat_dict[x])
        cape = np.cov(y, w)[0, 1] / (np.cov(y, w)[0, 0] + 1e-6)
    ...
    def growDecisionTreeFrom(self, X, treatment, y, evaluationFunction, max_depth=10,
        ...
        elif evaluationFunction == self.evaluate_Cape:
            currentScore = evaluationFunction(y, treatment)
        else:
            ...
            for value in lsUnique:
                ...
                if evaluationFunction == self.evaluate_CTS:
                    ...
                    if (self.control_name in leftNodeSummary and
                        ...
                        if evaluationFunction == self.evaluate_Cape:
                            leftScore1 = evaluationFunction(y_l,w_l)
                            rightScore2 = evaluationFunction(y_r,w_r)
                        else:
...
class UpliftRandomForestClassifier:
    ...
    def multiple_thread_rf(self, tree_i=None, tree=None, X=None, treatment=None, y=None):
        return tree_i, tree.fit(X, treatment, y)

    def predict(self, X, is_generate_json=True, n_jobs=None):
        '''
        Returns the recommended treatment group and predicted optimal
        probability conditional on using the recommended treatment group.
