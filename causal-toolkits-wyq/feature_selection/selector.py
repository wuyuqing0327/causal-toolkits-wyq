...
class Selector():
    ...
    def identify_zero_importance(self, task, eval_metric=None,
        ...
        for _ in range(n_iterations):
            ...
            if task == 'classification':
                model = lgb.LGBMClassifier(n_estimators=300, max_depth=6, learning_rate=0.08, random_state=66,
                                   subsample=0.75, is_unbalance=True)
                # model = xgb.XGBClassifier(n_estimators=1, max_depth=3, learning_rate=0.08, random_state=66,
                #                            subsample=0.75, min_child_weight=400, gamma=10)

            elif task == 'regression':
                model = lgb.LGBMRegressor(n_estimators=300, learning_rate=0.05)
                # model = xgb.XGBRegressor(n_estimators=1, learning_rate=0.08)
            ...
            if early_stopping:
                ...
                model.fit(train_features, train_labels, eval_metric=eval_metric,
                          eval_set=[(valid_features, valid_labels)]) #early_stopping_rounds=100,
    ...
    def identify_K_best(self, K=50):
        """
            Finds features top K best features
            combine 'chi-square' K-best and 'RFE' K-best

            return:
                array of K-best features
        """
        chisq_selector = SelectKBest(chi2, k=K)
        chisq_selector.fit(self.data, self.labels)
        clf = RandomForestClassifier(n_jobs=2, random_state=0)
        rfe = RFE(clf, K)
        rfe.fit(self.data, self.labels)
        chisq_K = chisq_selector.get_support()
        rfe_K = rfe.get_support()
        return np.union1d(chisq_K, rfe_K)
