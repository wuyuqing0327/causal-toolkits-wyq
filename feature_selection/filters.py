...
#             return 
    ...
    def _filter_D_one_feature(self, data, feature_name, y_name,
        ...
        x_bin = pd.qcut(data[feature_name].values, n_bins, labels=False, duplicates='drop')
        d_children = 0
        len_bin = len(set(x_bin))
        
#         cutPoint = np.unique(np.percentile(data[feature_name].values, range(n_bins, 100, n_bins)))
#         x_bin_ = pd.cut(data[feature_name].values, [-np.inf] + list(cutPoint) + [np.inf]) #,labels=False)
        
        data['x_bin'] = x_bin
        experiment_group_column_ = str(experiment_group_column)
        xbin_group = data.groupby(['x_bin',experiment_group_column_,y_name],as_index=False).agg({y_name:'count'})
        len_experiment_group_column = len(data[experiment_group_column_].unique())
#         print("xbin_group:", xbin_group)
        if (len_bin*len_experiment_group_column*2)!=xbin_group.shape[0]:
#             print("unoverlap_feature:", feature_name)
            D_result = pd.DataFrame({
            'feature': feature_name,
            'method': None,
            'score': None,
            'p_value': None,
            'misc': None}, index=[0]).reset_index(drop=True)
            return (D_result)
...
#             print("one_result:", one_result)
            if one_result['score'][0]==None:
                removed_features.append(x_name_i)
#                 print("removed_features_tmp:", removed_features)
                pass
            all_result = pd.concat([all_result, one_result],axis=0)
#             print("all_result:", all_result)
        print("removed_unoverlap_features_num_is_%s:" %(len(removed_features)),removed_features)
        all_result = all_result[~np.in1d(all_result['feature'],removed_features)]
        all_result = all_result.sort_values(by='score', ascending=False)
        all_result['rank'] = all_result['score'].rank(ascending=False)
#         print("all_result['rank']:", all_result[['rank','score']])
        
        return all_result

    def filter_P(self, data, features, y_name, k):
        df = data[features + [y_name]]
        res = np.abs(df.corr())
        vif_value = []
        ...
        if k >= len(var):
            ...
            for i in range(len(var)):
                for j in range(i+1, len(var)):
                    vif_value.append([df.columns[i], df.columns[j], res.iloc[i, j]])
            vif_value = sorted(vif_value, key=lambda x: x[2])
            print("max vif_value:", max(vif_value))

            for i in range(1, len(vif_value) + 1):
                var1 = vif_value[-i][0]
                var2 = vif_value[-i][1]
                if (var1 in var) & (var2 in var):
                    r = vif_value[-i][2]
                    if res[y_name][var1] > res[y_name][var2]:
                        var.remove(var2)
                    else:
                        var.remove(var1)
                if len(var) == k:
