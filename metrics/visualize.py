...
def get_uplift(df, outcome_col='gmv', treatment_col='is_treated', predict_col='predict', cost_col='cost',
               normalize=True, random_seed=42):
    assert ((outcome_col in df.columns) and (treatment_col in df.columns)
            and (predict_col in df.columns) and (cost_col in df.columns))
    ...
    acc_gain = df['cumsum_y_tr'] - df['cumsum_y_ct'] * df['cumsum_tr'] / df['cumsum_ct']
    acc_cost = df[cost_col].cumsum()
    qini.append(acc_cost)
    qini.append(acc_gain)

    qini = pd.concat(qini, join='inner', axis=1)
    qini.loc[0] = np.zeros((qini.shape[1],))
    qini = qini.sort_index().interpolate()

    qini.columns = ['acc_cost', 'acc_gain']
...
def plot_uplift(df, outcome_col='gmv', treatment_col='is_treated', predict_col='predict', cost_col='cost',
                normalize=False, random_seed=42, n=100, figsize=(8, 8)):
    plot(df, kind='uplift', n=n, figsize=figsize, outcome_col=outcome_col, treatment_col=treatment_col,
         predict_col=predict_col, cost_col=cost_col, normalize=normalize, random_seed=random_seed)
...
def auuc_score(df, outcome_col='y', treatment_col='w', treatment_effect_col='tau', normalize=True):
    ...
    auuc = cumgain.sum() / cumgain.shape[0]
    return auuc
