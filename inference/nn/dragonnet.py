...
from inference.nn.utils import (
...
from inference.meta.utils import convert_pd_to_np
...
class DragonNet:
    ...
    def fit(self, X, treatment, y, p=None):
        ...
        # y = np.hstack((y.reshape(-1, 1), treatment.reshape(-1, 1))) # singel_treatment
        y = np.hstack((y.reshape(-1, 1), treatment))  # multi_treatment

        self.dragonnet = self.make_dragonnet(X.shape[1])

        metrics = [regression_loss, treatment_accuracy, track_epsilon]
